...
CREATE SCHEMA IF NOT EXISTS `postagem` DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci;
USE `postagem` ;

CREATE TABLE IF NOT EXISTS `postagem`.`user` (
...
CREATE TABLE IF NOT EXISTS `postagem`.`categoryPost` (
...
CREATE TABLE IF NOT EXISTS `postagem`.`post` (
  ...
  CONSTRAINT `fkfk_categoryPost_post`
    ...
    REFERENCES `postagem`.`categoryPost` (`id_categoryPost`)
    ON DELETE NO ACTION
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
